...
2. Lägg till ditt MVC-projekt till din solution med Add existing project
3. Lägg till **EFCore Sqlite** till EventHomepage så att du kan konfigurera en databas i Program.cs
4. Se till så att projektet refererar EventCore och EventInfrastructure så att du kan använda dina domänmodeller och databasen
5. Fixa migrations:
...
6. Börja med att skapa API-endpointen för att lägga till events, så att du kan testa hela vägen från request -> controller -> dbcontext -> databas. Detta kan du göra med en enkel MapPost i program.cs eller så skapar du en controller, eller så gör du extensionmethods som vi testade tidigare.
...
7. Uppdatera HomeControllern och dess Index-vy så att du kan lista alla events på startsidan. Visa namn, tid, plats, om det är fullt eller inte, kanske hur många platser som är kvar. GÖR DET INTE SNYGGT! Fokusera på att få in datan och visa den, så kan du fixa designen senare.
...
8. Fixa en knapp/länk för att gå till den detaljerade vyn för eventen.
...
9. Den detaljerade vyn ska ha en formulär där man kan registrera sig för ett event. För att se hur detta sker i domänen måste du undersöka Event.cs. Du måste nu sätta upp en Action som kan ta emot formulärens data. 
